<p align="center">
  <img src="./assets/readme/hero.svg" width="100%" alt="复利 CI —— 把学习变成可持续积累、即时获得回报的复利游戏">
</p>

**复利（Compound Interest，CI）** 是一款个人使用的游戏化 AI 学习日程 Android 应用，为横屏平板设计，同时自适应手机竖屏等窄窗口。它把学习任务、确定性排程、专注计时、CI 币经济和数据复盘放进同一个**本地优先**的工作流：今天投入的每一分钟专注，都会立刻变成看得见的币、经验和头衔，再反哺明天的计划。

> 当前版本 `1.0.0` · 个人持续开发中，尚未上架应用商店。
...
六个页面：**今日**（时间线与专注操作）、**日程**（周视图与时间块编辑）、**任务**（领域 / 主线 / 支线管理与学习路线生成）、**商城**（奖励商品与每日精选）、**复盘**（统计图表与 CSV 导出）、**设置**（主题、模型路由与数据备份）。

### 平板与手机自适应布局

- 以 600dp 为断点区分宽屏 / 紧凑两种窗口尺寸：平板保持左侧导航栏与多栏布局，窄屏（含高度不足的横屏手机）自动切换为底部导航栏；
- 各页面按窗口重排：今日页折叠为「时间线 / 流水」单栏切换，商城与统计面板按可用宽度收缩，编辑弹窗填满宽度并保留边缘留白；
- 根布局统一消费 safeDrawing insets，状态栏与手势条不再遮挡内容。
...
语音不再只是录入，而是一层完整的操作入口：

- 内置 sherpa-onnx + SenseVoice-Small（int8）离线语音识别，说完即识、闲置自动释放；
- **16 个可注册语音技能**：开始 / 结束 / 放弃专注，完成 / 跳过 / 删除任务，主线支线的完成 / 归档 / 恢复 / 删除，日程与领域进度查询，商城查询与购买，时间块占用，六个页面导航；
- **双路由解析**：规则层按登记顺序先命中（触发词互不重叠，支持中文时间与拼音匹配）；兜不住时才交给 LLM 在全部技能说明里裁决，并对参数做候选校验——拒绝编造，校验不过退化为「未识别」，绝不静默执行；
- 每次执行前弹出确认卡片预览，删除、扣 CI 币等危险操作带警示样式；未识别时可直接手改文字，规则层实时重新匹配；
- 新增能力 = 写一个 `AppSkill` 接口实现 + 在 `SkillRegistry` 登记一行，解析器、路由、执行链路都不用动。
...
- Kotlin 2.1 · Jetpack Compose（Material 3）· Glance 1.1，单模块 `:app`；以 600dp 断点提供平板（侧边导航栏）/ 手机（底部导航栏）双形态布局；
...
- 32 个 JUnit4 JVM 测试类覆盖排程、经济、打卡、商城、导入导出、复盘聚合、语音技能路由与全部技能、LLM 路由与本地模型下载，测试方法按约定使用中文反引号命名。
...
`assembleRelease` 同样可以构建；个人自用阶段 release 构建复用 debug 签名，产出的 APK 可直接安装，上架前再替换为正式签名。
...
1. 在「任务」创建领域、主线或支线，再创建当天任务；
2. 在「今日」开始专注，结束后选择结果并结算 CI 币与经验（也可以直接按住语音按钮说「开始专注」）；
...
- 语音识别全程离线；语音指令仅在规则层未命中、且已配置模型路由时，才把识别文本交给模型裁决（本地路由不出设备）；
...
│   ├── core/          # 数据库、Repository、排程、经济、备份、导入导出、语音技能、设计系统
